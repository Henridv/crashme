...
int apairl(const void *x, const void *y)
{
    const struct apair *a = (const struct apair *)x;
    const struct apair *b = (const struct apair *)y;
    if (a->addr < b->addr) return (-1);
    else if (a->addr > b->addr) return (1);
    else return (0);
}

int adeltl(const void *x, const void *y)
{
    const struct adelt *a = (const struct adelt *)x;
    const struct adelt *b = (const struct adelt *)y;
